...
new_poor_scenarios <- pp_microsim_cc %>% 
  group_by(NAM_1) %>% 
  mutate(poor_baseline = poor_Avpovln2022_2030_baseline,
         new_poor_lab_cc = if_else(poor_Avpovln2022_2030_baseline==0 &
                                 poor_2030_baseline_lab_cc_avg==1, 1, 0),
         new_poor_lab_cc_foodPI = if_else(poor_Avpovln2022_2030_baseline==0 & poor_2030_baseline_lab_cc_foodPI ==1, 1, 0)) %>% 
  summarise(total_population = round(sum(weight_2030_baseline, na.rm = TRUE)),
            poor_baseline = 
              round(sum(poor_baseline*weight_2030_baseline, na.rm = TRUE)),
            new_p_lab_cc = 
              round(sum(new_poor_lab_cc*weight_2030_baseline, na.rm = TRUE)),
            new_p_lab_cc_foodPI = 
              round(sum(new_poor_lab_cc_foodPI*weight_2030_baseline, na.rm = TRUE))
            )
