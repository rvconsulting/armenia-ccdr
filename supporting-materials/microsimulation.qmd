...
We have that the "Min Year" is `{r} minyear` and the "Max Year" is `{r} maxyear`. Now we create a population growth rate by demographic variant dataset. We initialize an empty list to store our data by variant and we loop over variants to create this list.
...
#  filter(rural_dummy ==1 & is_ag_home == "Ag. HH (>= 50%)") %>%  
  ...
  group_by(poor_original, poor_b_energy) %>% 
  summarize(no_pp = sum(weight_2030_baseline, na.rm = TRUE),
            ...
            rural = sum(rural_dummy *weight_2030_baseline, na.rm = TRUE),
            urban = no_pp - rural
            )
...
income_columns <- names(pp_microsim_cc_pov)[grepl("^aec_r", names(pp_microsim_cc_pov))]

# Create a function to calculate both measures
calculate_measures <- function(column) {
  income <- pp_microsim_cc_pov[[column]]
  fgt1 <- Foster(income, poverty_line, parameter = 1)
  fgt2 <- Foster(income, poverty_line, parameter = 2)
  gini <- Gini(income, corr = FALSE, na.rm = TRUE)
  return(data.frame(
...
calculate_measures <- function(Income_Variable, Weight_Variable) {
  ...
  gini <- weighted.gini(income, w = weight)
