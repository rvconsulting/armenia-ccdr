...
# variants <- c("Medium", "Low", "High", "Constant-fertility", 
...
for (variant in variants) {
  for (t in minyear:maxyear) {
    ...
    pop_growth[[paste0("pop_growth_", t, "_", variant)]] <- pop_t / pop_base
  }
...
# Define the names of the scenarios and the variants to match your specific file and scenarios.
...
for (i in seq_along(scenarios)) {
  sheet_data <- read_excel(scenario_file, sheet = scenarios[i])
  sheet_data$scenid <- i
...
combined_data <- combined_data %>%
  mutate(rwage = wages_th_lcu / cpi)

# Assign variants to each scenid
combined_data <- combined_data %>%
  mutate(Variant = variants[scenid])
...
final_data <- combined_data %>%
  left_join(pop_proj_processed, by = c("year", "Variant"))

# Sort the final data by scenid and year
final_data <- final_data %>%
  arrange(scenid, year)

# Calculate consumption per capita
final_data <- final_data %>%
  mutate(consumption_pc = consumption_lcu / (pop * 10^2))

# Now, `final_data` contains the combined and computed data from all scenarios
