...
test <- test%>%
  ...
  group_by(poor_original, poor_foodPI) %>% 
  summarise(no_hh = round(sum(weight_2030_baseline, na.rm = TRUE)),
            no_pp = round(sum(weight_2030_baseline*hhsize, na.rm = TRUE)))
...
ggplot(ca_microsim_cc, 
       aes(x = aec_r_2030_baseline_foodPI, fill = 'Baseline + CC + Food Price (D/H)')) +
  ...
  geom_density(
    data = ca_microsim_cc,
    aes(x = aec_r_2030_baseline_lab_cc_avg, fill = 'Baseline + CC'),
    alpha = 0.5) +
  geom_density(
    data = ca_microsim_cc,
...
ggplot(ca_microsim_cc, 
       aes(x = aec_r_2030_baseline_foodPI, color = 'Baseline + CC + Food Price (D/H)')) +
  stat_ecdf(geom = "step") +
  # stat_ecdf(data = ca_microsim_cc, 
  #           aes(x = aec_r_2030_dry_hot, color = 'Dry/Hot')) +
  stat_ecdf(data = ca_microsim_cc, 
            aes(x = aec_r_2030_baseline_lab_cc_avg, color = 'Baseline + CC')) +
  stat_ecdf(data = ca_microsim_cc, 
