...
```{r}
#| label: import-economic-activity-codes
...
```{r}
#| label: plot-armenia-map-example
...
```{r}
#| label: import-labor-productivity-data
...
```{r warning=FALSE, message=FALSE}
#| label: import-vulnerability-data
...
wrf_2030_nzs       <- c(1.153117879, 1.217975434, 1.226770578)
...
ic_microsim <- ic %>% 
  ...
  mutate(
    ...
    totinc_coef_2030_nzs = 
      if_else(
        totalinc_2022 == 0, 
        1, 
        totalinc_2030_nzs / totalinc_2022)
    ) %>% 
  mutate(
    totinc_coef_2030_baseline = 
      if_else(
...
# #  group_by(gender) %>%
#   summarize(total_pp = sum(weight_2030_nzs, na.rm = TRUE)) %>%
#   ungroup()
# 
# table %>%
#   gt() %>%
...
ca_microsim <- ca %>% 
  ...
  mutate(
    ...
    totc_2030_nzs      = totc_2022 * totinc_coef_2030_nzs
    )
```
...
test <- ca_microsim_cc %>%
  ...
  summarize(no_hh = round(sum(weight_2030_dry_hot, na.rm = TRUE)),
...
# Basic density plot comparing food1 and food_exp_sim
ggplot(ca_microsim_cc, aes(x =  hous_23, fill = 'Initial LG')) + 
  geom_density(alpha = 0.3) + 
  geom_density(
    data = ca_microsim_cc,
   ...
   geom_density(
    data = ca_microsim_cc,
    aes(x = electricity_nzs_sim, fill = 'NSZ + NSZ Energy Prices'),
    alpha = 0.3) +
  labs(
    fill = "Consumption Type", 
...
# Adjust total expenditure (totc) based on the purchasing power loss
ca_microsim_cc <- ca_microsim_cc %>%
  mutate(totc_2030_baseline_energy = totc_2030_baseline * (1 - ttl_PPloss_1),
         totc_2030_nzs_energy = totc_2030_nzs * (1 - ttl_PPloss_2),
         )

# View the results
print(ca_microsim_cc %>% select(decile, totc_2030_baseline, totc_2030_baseline_energy, totc_2030_nzs_energy))
...
# Plot the cumulative distribution with left-facing arrows
ggplot(ca_microsim_cc, 
       aes(x = aec_r_2030_nzs_energy, color = 'NZS Energy Price')) +
  stat_ecdf(geom = "step") +
  # stat_ecdf(data = ca_microsim_cc, 
  #           aes(x = aec_r_2030_baseline_lab_avg, color = 'Baseline + Labor Productivity')) +
  stat_ecdf(data = ca_microsim_cc, 
            aes(x = aec_r_2030_baseline_energy, color = 'Baseline + NZS Energy Price')) +
  stat_ecdf(data = ca_microsim_cc, 
            aes(x = aec_r_2030_baseline, color = 'Baseline')) +
  labs(
    color = "Scenario Variant", 
...
#  filter(rural ==1 & is_ag_home == "Ag. HH (>= 50%)") %>%  
  ...
  group_by(poor_nzs, poor_nzs_energy) %>% 
  summarize(no_pp = sum(weight_2030_baseline, na.rm = TRUE),
            female = sum(female*weight_2030_baseline, na.rm = TRUE),
            male = no_pp - female,
            youth = sum(youth*weight_2030_baseline, na.rm = TRUE),
            non_youth = no_pp - youth,
            rural = sum(rural *weight_2030_baseline, na.rm = TRUE),
            urban = no_pp - rural
            )
...
new_poor_scenarios <- pp_microsim_cc %>%
  mutate(
    ...
    poor_nzs = poor_Avpovln2022_2030_dry_hot,
