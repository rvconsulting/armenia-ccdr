...
writeClipboard(as_raw_html(gt(pop_targets_2030)))
...
anesrakefinder(targets_baseline, rakedata, choosemethod = "total")

outsave <- anesrake(targets_baseline, 
                    rakedata, 
                    caseid = rakedata$pid, 
                    #verbose = FALSE,
                    choosemethod = "total",
...
anesrakefinder(targets_dry_hot, rakedata, choosemethod = "total")

outsave <- anesrake(targets_dry_hot, 
                    rakedata, 
                    caseid = rakedata$pid, 
                    #verbose = FALSE,
                    choosemethod = "total",
  ...
  mutate(
    mli_2030_baseline_lab_avg =
      case_when(
        ...
        sector == 1 ~
          monthly_labor_income_2030_baseline * (1 + agr_avg) * 1000,
        sector == 2 ~
          monthly_labor_income_2030_baseline * (1 + man_avg) * 1000,
        sector == 3 ~
          monthly_labor_income_2030_baseline * (1 + ser_avg) * 1000,
        TRUE ~ NA
      )
  ) %>%
  mutate(
    mli_2030_baseline_lab_max =
      case_when(
        # * 1000 because its thousands of Dram
        sector == 1 ~
          monthly_labor_income_2030_baseline * (1 + agr_max) * 1000,
        sector == 2 ~
          monthly_labor_income_2030_baseline * (1 + man_max) * 1000,
        sector == 3 ~
          monthly_labor_income_2030_baseline * (1 + ser_max) * 1000,
        TRUE ~ NA
      )
  )
```
...
ic_new_incomes <- pp_microsim_cc %>%
  ...
  summarise(
    ...
    mli_original = 
      sum(monthly_labor_income_2022 * 1000, na.rm = TRUE)
  ) %>%
  mutate(
...
And now we impact agricultural income `inc4`.

```{r}
ic_microsim_cc <- ic_microsim_cc %>% 
  mutate(inc4_2030_baseline_cc_avg = inc4 * (1 + crops_avg_loss),
         inc4_2030_baseline_cc_max = inc4 * (1 + crops_max_loss))
```
  ...
  mutate(
    totalinc_2030_baseline_lab_avg_coef =
      if_else(totalinc_2030_baseline == 0, 
              1, totalinc_2030_baseline_lab_avg / totalinc_2030_baseline),
    totalinc_2030_baseline_lab_max_coef =
      if_else(totalinc_2030_baseline == 0, 
              1, totalinc_2030_baseline_lab_max / totalinc_2030_baseline)
  ) %>%
  mutate(
    totalinc_2030_baseline_lab_avg_coef =
...
We do the same for ag income alone
...
ic_microsim_cc <- ic_microsim_cc %>%
  mutate(
    totalinc_2030_baseline_cc_avg =
      totalinc_2030_baseline -
      rowSums(select(., c(inc4)), na.rm = TRUE) +
      rowSums(select(
        ., c(inc4_2030_baseline_cc_avg)), na.rm = TRUE),
    totalinc_2030_baseline_cc_max =
      totalinc_2030_baseline -
      rowSums(select(., c(inc4)), na.rm = TRUE) +
      rowSums(select(
        ., c(inc4_2030_baseline_cc_max)), na.rm = TRUE)
  ) %>%
  mutate(
  ...
  mutate(
    totalinc_2030_baseline_lab_cc_avg =
      ...
      rowSums(select(
        ., c(inc2_2030_baseline_lab_avg, 
             inc3_2030_baseline_lab_avg,
             inc4_2030_baseline_cc_avg)), na.rm = TRUE),
    totalinc_2030_baseline_lab_cc_max =
      ...
      rowSums(select(
        ., c(inc2_2030_baseline_lab_max, 
             inc3_2030_baseline_lab_max,
             inc4_2030_baseline_cc_max)), na.rm = TRUE)
  ) %>%
  mutate(
    totalinc_2030_baseline_lab_cc_avg_coef =
      if_else(totalinc_2030_baseline == 0, 
              1, totalinc_2030_baseline_lab_cc_avg / totalinc_2030_baseline),
    totalinc_2030_baseline_lab_cc_max_coef =
      if_else(totalinc_2030_baseline == 0, 
              1, totalinc_2030_baseline_lab_cc_max / totalinc_2030_baseline)
  ) %>%
  mutate(
    totalinc_2030_baseline_lab_cc_avg_coef =
...
ca_microsim_cc <- ca_microsim_cc %>% 
  mutate(totc_2030_baseline_lab_avg = totc * totalinc_2030_baseline_lab_avg_coef,
...
# View the results
print(ca_microsim_cc %>% select(decile, food1, food_1_2030, food_1_dprice, food_q1_sim, food_exp1_sim, food_2_dprice, food_q2_sim, food_exp2_sim))
...
# View the results
print(ca_microsim_cc %>% select(decile, food1, food_1_2030, food_1_dprice, food_q1_sim, food_exp1_sim, food_2_dprice, food_q2_sim, food_exp2_sim, food1_CSloss, food2_CSloss, ttl_CSloss_1, ttl_CSloss_2, food1_PPloss, food2_PPloss, ttl_PPloss_1, ttl_PPloss_2, totc_cc_avg_food1, totc_cc_avg_food2))
...
ca_microsim_cc <- ca_microsim_cc %>%
  ...
  mutate(poor_cc_avg_food1 =
         ...
         poor_cc_avg_food2 =
           if_else(aec_r_cc_avg_food2 < 52883, 1, 0))
...
test <- test%>%
  ...
  group_by(poor_original, poor_cc_food1) %>% 
  summarise(no_hh = round(sum(weight, na.rm = TRUE)),
            no_pp = round(sum(weight*hhsize, na.rm = TRUE)))
...
ggplot(ca_microsim_cc, 
       aes(x = aec_r_cc_avg_food1, fill = 'Dry/Hot + Food Price')) +
  geom_density(alpha = 0.5) + 
  geom_density(
    data = ca_microsim_cc, 
    aes(x = aec_r_cc_avg_food2, fill = 'Baseline + CC + Food Price'), 
    alpha = 0.5) + 
  geom_density(
    data = ca_microsim_cc, 
    aes(x = aec_r_cc_avg, fill = 'Baseline + CC'), 
    alpha = 0.5) + 
  geom_density(
    data = ca_microsim_cc, 
    aes(x = aec_r, fill = 'Baseline'), 
    alpha = 0.5) +
  labs(
    fill = "Consumption Type", 
...
ggplot(ca_microsim_cc, 
       aes(x = aec_r_cc_avg_food1, color = 'Dry/Hot + Food Price')) +
  stat_ecdf(geom = "step") +
  stat_ecdf(data = ca_microsim_cc, 
            aes(x = aec_r_cc_avg_food2, color = 'Baseline + CC + Food Price')) +
  stat_ecdf(data = ca_microsim_cc, 
            aes(x = aec_r_cc_avg, color = 'Baseline + CC')) +
  stat_ecdf(data = ca_microsim_cc, 
            aes(x = aec_r, color = 'Baseline')) +
  labs(
...
test <- test%>%
  ...
  summarise(no_pp = round(sum(weight, na.rm = TRUE)),
            female = round(sum(female*weight, na.rm = TRUE)),
            male = no_pp - female,
            youth = round(sum(youth*weight, na.rm = TRUE)),
            non_youth = no_pp - youth,
            rural = round(sum(rural *weight, na.rm = TRUE)),
            urban = no_pp - rural)
...
new_poor_scenarios <- pp_microsim_cc %>% 
  group_by(NAM_1) %>% 
  mutate(new_poor_cc = if_else(poor_Avpovln2022==0 & poor_cc_avg==1, 1, 0),
         new_poor_cc_food1 = if_else(poor_Avpovln2022==0 & poor_cc_avg_food1 ==1, 1, 0),
         new_poor_cc_food2 = if_else(poor_Avpovln2022==0 & poor_cc_avg_food2 ==1, 1, 0)) %>% 
  summarise(total_population = round(sum(weight, na.rm = TRUE)),
            new_p_cc = round(sum(new_poor_cc*weight, na.rm = TRUE)),
            new_p_food1 = round(sum(new_poor_cc_food1*weight, na.rm = TRUE)),
            new_p_food2 = round(sum(new_poor_cc_food2*weight, na.rm = TRUE)),
            new_p_cc_pct = new_p_cc/total_population * 100,
            new_p_food1_pct = new_p_food1/total_population * 100,
            new_p_food2_pct = new_p_food2/total_population * 100
            )

new_poor_map <- adm1 %>% 
  left_join(new_poor_scenarios, join_by(NAM_1)) %>% 
  mutate(
     new_p_cc_label    = paste0(NAM_1,"\n(", sprintf("%.1f%%",new_p_cc_pct   ), ")"),
     new_p_food1_label = paste0(NAM_1,"\n(", sprintf("%.1f%%",new_p_food1_pct), ")"),
     new_p_food2_label = paste0(NAM_1,"\n(", sprintf("%.1f%%",new_p_food2_pct), ")")
  )
...
tm_shape(new_poor_map)+
  tm_polygons("new_p_cc", legend.show = FALSE) +
  tm_text(c("new_p_cc_label"), size = .7, col = "black")+
  tm_layout(legend.position = c("right", "top"), 
            title= "New poor as a share of total population\nBaseline + CC", 
            title.position = c('left', 'bottom'),
            title.size = 0.9)

tm_shape(new_poor_map)+
  tm_polygons("new_p_food2_pct", legend.show = FALSE) +
  tm_text(c("new_p_food2_label"), size = .7, col = "black")+
  tm_layout(legend.position = c("right", "top"), 
            title= "New poor as a share of total population\nBaseline + CC + FP", 
            title.position = c('left', 'bottom'),
            title.size = 0.9)
...
tm_shape(new_poor_map)+
  tm_polygons("new_p_food1_pct", legend.show = FALSE) +
  tm_text(c("new_p_food1_label"), size = .7, col = "black")+
  tm_layout(legend.position = c("right", "top"), 
            title= "New poor as a share of total population\nDry-Hot Scenario + FP", 
            title.position = c('left', 'bottom'),
            title.size = 0.9)
...
avg_scenario_losses <- ca_microsim_cc %>% 
  mutate(totc_loss_cc = if_else((totc_cc_avg - totc)< 0,
                                (totc_cc_avg - totc)/totc,NA),
         totc_loss_cc_food1 =
           if_else(
             (totc_cc_avg_food1 - totc)< 0,
             (totc_cc_avg_food1 - totc)/totc,NA),
         totc_loss_cc_food2 = 
           if_else(
             (totc_cc_avg_food2 - totc)< 0,
             (totc_cc_avg_food2 - totc)/totc,NA)
           ) %>%
  group_by(decile) %>% 
  summarise(no_hh = 
              round(
                sum(
                  weight, na.rm = TRUE), digits = 0
                ),
            avg_totc = 
              round(
                weighted.mean(
                  totc, weight, na.rm = TRUE), digits = 2
                ),
            avg_totc_usd = 
              round(
                weighted.mean(
                  totc, weight, na.rm = TRUE)*er, digits = 1
                ),
            avg_loss_cc = 
              round(
                weighted.mean(
                  totc_loss_cc, weight, na.rm = TRUE), digits = 4
                ),
            avg_loss_cc_food1 = 
              round(
                weighted.mean(
                  totc_loss_cc_food1, weight, na.rm = TRUE), digits = 4
                ),
            avg_loss_cc_food2 = 
              round(
                weighted.mean(
                  totc_loss_cc_food2, weight, na.rm = TRUE), digits = 4
                )
            )
