...
livestock_productivity <-
  read.csv("data/ARM-Vulnerability-Analysis/ARM_livestock_REF_shock.csv")
crops_aal <- 
  read_xlsx("data/ARM-Vulnerability-Analysis/Data_AAL_AAE.xlsx",
...
And we do the same for livestock productivity.In this case, there is no disaggregation by Marz.
...
# Clim scenarios to select
cs <- unique(livestock_productivity$climate_scenario)

# Moving average for year of interest
lvstk_pdcvty_loss <- livestock_productivity %>%
  filter(climate_scenario == cs[1], 
         year == yearsto[1]) %>%
  select(-pct_change_rev, 
         #-year, 
         -climate_scenario) %>%
...
ic_microsim_cc <- ic_microsim %>%
  left_join(ag_pdcvty_loss, 
            join_by(NAM_1 == NAM_1)) %>% 
  mutate(lvstk_avg_loss = lvstk_pdcvty_loss$lvstk_avg_loss[1],
         lvstk_max_loss = lvstk_pdcvty_loss$lvstk_max_loss[1])
```
